...
class SubcountScreen:
    def __init__(self, config, modules, default_actions):
        ...
        self.name_color = literal_eval(
            config.get("Youtube", "name_color", fallback="(255,255,255)")
        )
        self.sub_color = literal_eval(
            config.get("Youtube", "sub_color", fallback="(255,255,255)")
        )
        ...
        if yt_token is None:
            ...
            if self.channel_id is None:
                ...
                self.display_name = config.get(
                    "Youtube", "display_name", fallback=self.channel_id
                )
            threading.Thread(
                target=fetchYoutubeSubsAsync,
                args=(self.queue, yt_token, self.channel_id),
            ).start()
    ...
    def generate(self, isHorizontal, inputStatus):
        ...
        if self.display_name == "bit of a ch.allen.ge":
            ...
            draw.text(
                (0, 18), self.display_name.upper(), self.name_color, font=self.font
            )
