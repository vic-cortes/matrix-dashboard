...
def placeText(frame, x, y, color, text_arr, isLeftReference):
    ...
    if isLeftReference:
        np.putmask(
            frame[y : y + arr_height, x : x + arr_width, 0],
            text_arr.astype(bool),
            values * color[0],
        )
        np.putmask(
            frame[y : y + arr_height, x : x + arr_width, 1],
            text_arr.astype(bool),
            values * color[1],
        )
        np.putmask(
            frame[y : y + arr_height, x : x + arr_width, 2],
            text_arr.astype(bool),
            values * color[2],
        )
    else:
        np.putmask(
            frame[y : y + arr_height, x - arr_width : x, 0],
            text_arr.astype(bool),
            values * color[0],
        )
        np.putmask(
            frame[y : y + arr_height, x - arr_width : x, 1],
            text_arr.astype(bool),
            values * color[1],
        )
        np.putmask(
            frame[y : y + arr_height, x - arr_width : x, 2],
            text_arr.astype(bool),
            values * color[2],
        )
...
def generateFrameAsync(queue, ticker_symbols, font, bg):
    while True:
        ...
        for i in range(len(ticker_symbols)):
            ...
            (arrow_color, arrow_arr) = (
                (green, up_arrow)
                if float(current) - float(last_close) >= 0
                else (red, down_arrow)
            )
