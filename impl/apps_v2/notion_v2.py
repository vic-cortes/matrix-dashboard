...
class NotionScreen:
    def __init__(self, config, modules, default_actions):
        ...
        self.text_color = literal_eval(
            config.get("Notion", "text_color", fallback="(255,255,255)")
        )
        self.todo_color = literal_eval(
            config.get("Notion", "todo_color", fallback="(255,100,140)")
        )
        self.doing_color = literal_eval(
            config.get("Notion", "doing_color", fallback="(255,202,0)")
        )
        ...
        if notion_token is None or notion_database_id is None:
            ...
            threading.Thread(
                target=fetchNotionAsync,
                args=(self.queue, notion_token, notion_database_id),
            ).start()
        ...
        if isHorizontal:
            ...
            if self.tasks is None or len(self.tasks) == 0:
                ...
                for i in range(len(self.tasks[0:5])):
                    task_desc = self.tasks[i]["properties"]["Name"]["title"][0][
                        "plain_text"
                    ].upper()
                    task_desc_len = self.font.getsize(
                        self.tasks[i]["properties"]["Name"]["title"][0][
                            "plain_text"
                        ].upper()
                    )[0]
                    ...
                    if task_desc_len >= self.canvas_width - 3:
                        spacer = "     "
                        draw.text(
                            (3 - self.animation_cnt[i], 6 * i),
                            task_desc + spacer + task_desc,
                            self.text_color,
                            font=self.font,
                        )
                        if not self.paused:
                        ...
                        draw.text(
                            (3, 6 * i), task_desc, self.text_color, font=self.font
                        )

                    task_status = self.tasks[i]["properties"]["Status"]["select"][
                        "name"
                    ]
...
def fetchNotionAsync(queue, token, databaseID):
    ...
    while True:
                ...
                "or": [
                    {
                        "and": [
                            {
                                "property": "Date Due",
                                "date": {"on_or_before": week_from_now.isoformat()},
                            },
                    ...
                    {"property": "Status", "select": {"equals": "Unassigned"}},
                    {
                        "property": "Date Due",
        ...
        res = requests.request(
            "POST", queryURL, headers=headers, data=json.dumps(query_params)
        )
