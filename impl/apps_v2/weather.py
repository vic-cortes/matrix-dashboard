...
class WeatherScreen:
    def __init__(self, config, modules, default_actions):
        ...
        self.text_color = literal_eval(
            config.get("Weather Screen", "text_color", fallback="(255,255,255)")
        )
        self.low_color = literal_eval(
            config.get("Weather Screen", "low_color", fallback="(255,255,255)")
        )
        self.high_color = literal_eval(
            config.get("Weather Screen", "high_color", fallback="(255,255,255)")
        )
    ...
    def generate(self, isHorizontal, inputStatus):
        ...
        if one_call is not None:
            ...
            if currentTime.hour > dtsr.hour and currentTime.hour <= dtss.hour:
                ...
                draw.text(
                    (17, 17),
                    str(hours) + ":" + convertToTwoDigits(dtss.minute),
                    self.text_color,
                    font=self.font,
                )
            else:
                ...
                draw.text(
                    (21, 17),
                    str(hours) + ":" + convertToTwoDigits(dtsr.minute),
                    self.text_color,
                    font=self.font,
                )
    ...
    for _, _, files in os.walk("apps_v2/res/weather"):
        for file in files:
            if file.endswith(".png"):
                icon_map[file[:-4]] = Image.open("apps_v2/res/weather/" + file).convert(
                    "RGB"
                )
    return icon_map
