...
class SpotifyScreen:
    def __init__(self, config, modules, default_actions):
        ...
        self.title_color = literal_eval(
            config.get("Spotify Player", "title_color", fallback="(255,255,255)")
        )
        self.artist_color = literal_eval(
            config.get("Spotify Player", "artist_color", fallback="(255,255,255)")
        )
        self.play_color = literal_eval(
            config.get("Spotify Player", "play_color", fallback="(255,255,255)")
        )
        ...
        if response is not None:
            ...
            if self.current_art_url != art_url:
                ...
                self.current_art_img = img.resize(
                    (self.canvas_height, self.canvas_height), resample=Image.LANCZOS
                )
            ...
            draw.line(
                (38, 15, 38 + round(((progress_ms / duration_ms) * 100) // 5), 15),
                fill=(180, 180, 180),
            )
            ...
            if title_len > 31:
                spacer = "   "
                draw.text(
                    (34 - self.title_animation_cnt, 0),
                    self.current_title + spacer + self.current_title,
                    self.title_color,
                    font=self.font,
                )
                self.title_animation_cnt += 1
                if (
                    self.title_animation_cnt
                    == self.font.getsize(self.current_title + spacer)[0]
                ):
                    self.title_animation_cnt = 0
            else:
                draw.text(
            ...
            if artist_len > 31:
                spacer = "     "
                draw.text(
                    (34 - self.artist_animation_cnt, 7),
                    self.current_artist + spacer + self.current_artist,
                    self.artist_color,
                    font=self.font,
                )
                self.artist_animation_cnt += 1
                if (
                    self.artist_animation_cnt
                    == self.font.getsize(self.current_artist + spacer)[0]
                ):
                    self.artist_animation_cnt = 0
            else:
                draw.text(
